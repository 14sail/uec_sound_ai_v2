...
def prune_layers(net, prune_targets, iterations, prune_all=True, device='cpu'):
    net = net.cpu();
    ...
    del net.tfeb;
    # net.tfeb = torch.nn.Sequential(*[module for idx, module in all_features[tfeb_starts_at:-1]]);
    # net.tfeb = torch.nn.Sequential(*);
    
    modules = [module for idx, module in all_features[tfeb_starts_at:-3]]
    modules.append(torch.nn.AvgPool2d(kernel_size=(5,7)))
    fcn = torch.nn.Linear(7, 7)
    modules.extend([torch.nn.Flatten(), fcn])
    net.tfeb = torch.nn.Sequential(*modules)
